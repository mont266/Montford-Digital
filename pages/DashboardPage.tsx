        ...
        <div className="space-y-10">
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <StatCard title="Stripe Fees" value={formatCurrency(totalStripeFees)} icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>} />
                </div>
            </div>
...
const ProjectsPage: React.FC<{ projects: Project[]; clients: any[]; refreshData: () => void; selectedEntityId: string }> = ({ projects, clients, refreshData, selectedEntityId }) => {
    ...
    const [workspaceModalProject, setWorkspaceModalProject] = useState<Project | null>(null);
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-3">
                    <button onClick={refreshData} title="Refresh Projects" className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded transition-colors flex items-center justify-center">
            ...
            {workspaceModalProject && <ProjectWorkspaceModal project={workspaceModalProject} onClose={() => setWorkspaceModalProject(null)} />}
        </div>
    );
};
                                    ...
                                    return (
                                        <div key={month} className={`flex items-center justify-between p-4 bg-slate-800 ${index !== array.length - 1 ? 'border-b border-slate-700' : ''}`}>
                                            ...
                                            <div className="flex items-center space-x-4">
                                                <span className="text-slate-300">{formData.currency} {(amount as number).toFixed(2)}</span>
